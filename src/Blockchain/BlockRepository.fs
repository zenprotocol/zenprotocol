    ...
    let blockState =
        {
            ...
            activeContractSet =
                ...
                |> Seq.map (fun contract -> contract.hash,contract.expiry,contract.size)
                |> List.ofSeq
        }
...
let getBlockState session blockHash =
    ...
    let acs =
        ...
        |> List.map (fun (cHash,expiry,size) -> cHash,Contract.load session.context.contractPath expiry size cHash)
        |> List.map getOk
        |> List.toArray
