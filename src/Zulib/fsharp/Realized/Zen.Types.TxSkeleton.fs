...
type txSkeleton =
    { // A map from asset identifiers to the total amount of that asset and the corresponding pointedOutputs
      inputs : Collections.Map<asset, (U64.t * list<pointedOutput>)>;
      // A map from asset identifiers to the total amount of that asset and the corresponding outputs
...
let insertPointedOutput (pointedOutput : pointedOutput) (txSkeleton : txSkeleton) : txSkeleton =
    let {asset=asset; amount=amount} = (snd pointedOutput).spend
    let inputs =
        if Map.containsKey asset txSkeleton.inputs
        then
            let oldAmount, pointedOutputs = Map.find asset txSkeleton.inputs
            txSkeleton.inputs |> Map.add asset (amount + oldAmount, pointedOutput::pointedOutputs)
        else
            txSkeleton.inputs |> Map.add asset (amount, [pointedOutput])
...
let insertOutput (output : output) (txSkeleton : txSkeleton) : txSkeleton =
    ...
    let outputs =
        ...
        then
            ...
            txSkeleton.outputs |> Map.add asset (amount + oldAmount, output::outputs)
        else
            txSkeleton.outputs |> Map.add asset (amount, [output])
    {txSkeleton with outputs=outputs}

let addInput (pointedOutput : pointedOutput) (txSkeleton : txSkeleton) : Cost.t<txSkeleton, unit> =
    lazy (insertPointedOutput pointedOutput txSkeleton )
    |> Cost.C
...
let addOutput (output : output) (txSkeleton : txSkeleton) : Cost.t<txSkeleton, unit> =
    lazy (insertOutput output txSkeleton)
    |> Cost.C
...
let mint (amount:U64.t) (contractHash:contractHash) (txSkeleton : txSkeleton)
    : Cost.t<txSkeleton, unit> =
    lazy (let mintSpend = {asset=contractHash; amount=amount}
          let mintLock = ContractLock(contractHash, 0I, Empty)
          let mintOutput = {lock=mintLock; spend=mintSpend}
          let mintTxHash = Array.create 32 0uy
          let mintOutpoint = {txHash=mintTxHash; index=0u}
...
let isValid (txSkeleton : txSkeleton) : Cost.t<bool, unit> =
    lazy ( if keySet txSkeleton.inputs <> keySet txSkeleton.outputs
           then false
           else
                txSkeleton.inputs
                |> Map.forall(fun asset (inputAmount, _) ->
                    let outputAmount, _ = Map.find asset txSkeleton.outputs
