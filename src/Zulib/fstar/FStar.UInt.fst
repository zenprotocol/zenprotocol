...
let max_int (n:nat) : int = pow2 n - 1
let min_int (n:nat) : int = 0

let fits (x:int) (n:nat) : bool = min_int n <= x && x <= max_int n
let size (x:int) (n:nat) : Type0 = b2t(fits x n)

(* Machine integer type *)
type uint_t (n:nat) = x:int{size x n}

(* Constants *)
val zero: n:nat -> uint_t n
let zero n = 0

val one: n:pos -> uint_t n
let one n = 1

val ones: n:nat -> uint_t n
let ones n = max_int n

(* Increment and decrement *)
val incr(#n:nat): x:uint_t n{x < max_int n} -> uint_t n
let incr(#_) x = x + 1

val decr(#n:nat): a:uint_t n{a > min_int n} -> uint_t n
let decr(#_) a = a - 1
...
val eq(#n:nat): uint_t n -> uint_t n -> bool
let eq #_ a b = (a = b)

val gt(#n:nat): uint_t n -> uint_t n -> bool
let gt #_ a b = (a > b)

val gte(#n:nat): uint_t n -> uint_t n -> bool
let gte #_ a b  = (a >= b)

val lt(#n:nat): uint_t n -> uint_t n -> bool
let lt #_ a b = (a < b)

val lte(#n:nat): uint_t n -> uint_t n -> bool
let lte #_ a b = (a <= b)

(* Casts *)
val to_uint_t: m:nat -> int -> uint_t m
