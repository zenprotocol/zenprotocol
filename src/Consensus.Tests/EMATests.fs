...
let ``Default difficulty for first 11 blocks``(tstamps:uint64 list) =
    let ema = create Chain.Main
    let emas = List.scan (fun e ts -> add Chain.Main ts e) ema (List.take 11 tstamps)
...
let ``Difficulty increases when blocks are generated quickly``(tstamps:uint64 list) =
    let ema = create Chain.Main
    let emas = List.scan (fun e ts -> add Chain.Main ts e) ema tstamps
